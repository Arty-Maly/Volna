...
class DataHandler: DataDelegate {
    ...
    init(dataHandlerDelegate dataDelegate: ErrorDelegate) {
    ...
    func syncRadioStations() {
        ...
        let request = RequestMaker(url: url, dataDelegate: self)
        request.getStations()
    }
    
    internal func matchLocalData(with data: Data) {
    ...
    private func prepareImageForSaving(radioStationId: NSManagedObjectID) {
        convertQueue.async(group: syncGroup) {
            ...
            let radioStation = imageManagedObjectContext.object(with: radioStationId) as! RadioStation
            guard let data = try? Data(contentsOf:  URL(string: radioStation.image)!),
